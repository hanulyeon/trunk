...
public class OlapModelInfo extends DataObject {
	private static final String DTO_LOGICAL_NAME = "OlapModelInfo";
	...
	public Object clone() {
		OlapModelInfo copyObj = new OlapModelInfo();	
		copyObj.clone(this);
		return copyObj;
	}
	public void clone(DataObject _olapModelInfo) {
		if(this == _olapModelInfo) return;
		OlapModelInfo __olapModelInfo = (OlapModelInfo) _olapModelInfo;
		
		this.setId(__olapModelInfo.getId());
		this.setName(__olapModelInfo.getName());
		this.setAuthor(__olapModelInfo.getAuthor());
		this.setLastEdited(__olapModelInfo.getLastEdited());
		this.setProjectId(__olapModelInfo.getProjectId());
		this.setContents(__olapModelInfo.getContents());
		this.setDescription(__olapModelInfo.getDescription());
	...
	static {
		...
		fieldPropertyMap.put("projectId"
							, FieldPropertyFactory.getFieldProperty( "projectId","", "", "", false, false, false,  FieldProperty.TYPE_PRIMITIVE_INT, 10 , -1, null, null));		
	...
	public boolean equals(Object obj) {
		...
		if ((obj instanceof OlapModelInfo) == false) return false;
		OlapModelInfo _OlapModelInfo = (OlapModelInfo) obj;				
		if(id != _OlapModelInfo.getId()) return false; 									
		if(name == null) {
			if(_OlapModelInfo.getName() != null)
				return false;
		} else if(!name.equals(_OlapModelInfo.getName()))
			return false;					
		if(author == null) {
			if(_OlapModelInfo.getAuthor() != null)
				return false;
		} else if(!author.equals(_OlapModelInfo.getAuthor()))
			return false;					
		if(lastEdited == null) {
			if(_OlapModelInfo.getLastEdited() != null)
				return false;
		} else if(!lastEdited.equals(_OlapModelInfo.getLastEdited()))
			return false;				
		if(projectId != _OlapModelInfo.getProjectId()) return false; 									
		if(contents == null) {
			if(_OlapModelInfo.getContents() != null)
				return false;
		} else if(!contents.equals(_OlapModelInfo.getContents()))
			return false;					
		if(description == null) {
			if(_OlapModelInfo.getDescription() != null)
				return false;
		} else if(!description.equals(_OlapModelInfo.getDescription()))
			return false;
		return true;
	}
